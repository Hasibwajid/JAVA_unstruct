...
public class Krushkal_MST {
    ...
    public void kruskalMST(ArrayList<Edge> graphEdges,int nodeCount) {
        ...
        Collections.sort(graphEdges);        //sort edges with the smallest weight 1st
        ...
        for (int i = 0; i < graphEdges.size() && mstEdges.size()
            ...
            outputMessage += "find(" + currentEdge.getVertex1() + ") returns " + v1 + ", find(" + currentEdge.getVertex2() + ") returns " + v2;        //just print, keep on same line for union message
...
class DisjointSet {
    ...
    public DisjointSet(int numElements) {        //constructor creates singleton sets
        ...
        Arrays.fill(set, -1);//initialize to -1 so the trees have nothing in them
//        OR
...
//        }
    ...
    public static void main(String[] args) {
        ...
        int nodeCount = 7;
        Krushkal_MST graph = new Krushkal_MST();
